...
Page({
  ...
  bindinputNumber(event) {
    ...
    address.integral = event.detail.value;
    this.setData({
      address: address
    });
  },
  ...
  bindinputName(event) {
    let address = this.data.address;
    address.shipping_name = event.detail.value;
    this.setData({
      address: address
    });
  },
  bindinputMoneny (event){
    let address = this.data.address;
    address.goods_price = event.detail.value;
    this.setData({
      address: address
    });
  },
  ...
  getAddressDetail() {
    let that = this;
    util.request(api.ShopDetail, { id: that.data.addressId }).then(function (res) {
      if (res.errno === 0) {
        if(res.data){
            that.setData({
                address: res.data
  ...
  saveAddress1(){
    ...
    if (address.shipping_name == '') {
      util.showErrorToast('请输入商品');

      return false;
    }

    if (address.integral == '') {
      util.showErrorToast('采购数量');
      return false;
    }
    if (address.mobile == '') {
      util.showErrorToast('手机号');
      return false;
    }


    if (address.goods_price == '') {
      util.showErrorToast('采购金额');
      return false;
    }


    let that = this;
    util.request(api.ShopSave, { 
      id: address.id,
      shipping_name: address.shipping_name,
      integral: address.integral,
      goods_price: address.goods_price,
      mobile: address.mobile,
      detailInfo: address.detailInfo,
    }, 'POST').then(function (res) {
      if (res.errno === 0) {
        console.log('32453243243-------------------------++++++++++++++++++==');
        wx.redirectTo({
          url: '/pages/ucenter/advOrder/advOrder?status=1',
        })
      }else{
        wx.redirectTo({
          url: '/pages/ucenter/advOrder/advOrder?status=0',
        })
      }
    });
